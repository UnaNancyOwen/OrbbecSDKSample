...
{
    // Start Recorder
    recorder = ob_create_recorder_with_device( device, &error );
    CHECK_ERROR( error );

    ob_recorder_start( recorder, bag_file.c_str(), true, &error );
    CHECK_ERROR( error );
}
...
void orbbec::write_frame()
{
    ...
    // Write Frame
    ob_recorder_write_frame( recorder, frameset, &error );
    CHECK_ERROR( error );
}
