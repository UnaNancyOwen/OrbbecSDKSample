...
inline void orbbec::initialize_sensor()
{
    ...
    // Get Stream Profile
    ob_stream_profile_list* stream_profile_list = ob_pipeline_get_stream_profile_list( pipeline, ob_sensor_type::OB_SENSOR_DEPTH, &error );
    CHECK_ERROR( error );

    depth_stream_profile = ob_stream_profile_list_get_video_stream_profile( stream_profile_list, 320, 288, ob_format::OB_FORMAT_Y16, 30, &error );
    if( ob_error_status( error ) != ob_status::OB_STATUS_OK ){
        error = NULL;
        depth_stream_profile = ob_stream_profile_list_get_profile( stream_profile_list, 0, &error ); // default
    }
    CHECK_ERROR( error );
